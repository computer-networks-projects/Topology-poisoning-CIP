...
log = core.getLogger()
link_latency = {}
latency = {} # for calculating zscore
...
class Discovery (EventMixin):
  ...
  def _expire_links (self):
    ...
    if expired:
      ...
      log.info("Currently available links: ")
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if (event.dpid, event.port) == (originatorDPID, originatorPort):
      log.warning("FAKE LINK DETECTED")

      switch_port = dpid_to_str(event.dpid)+"."+str(event.port)
      log.info("port id: %s  <-- MIGHT BE COMPROMISED",switch_port)

      for key,value in self.adjacency.items():
          if(str(switch_port) in str(key)):
              log.warning("%s|%s   <-- MIGHT BE A FAKE LINK",key,value)
    ...
    for key,value in list(link_latency.items()):
        if len(key)==4:
            continue
        if len(key)==2 and key[0]==arr1[0] and key[1]==arr1[1]: #match
            array=list(key)
    ...
    for key,value in link_latency.items():
        if len(key)==4 and type(value)==float:
            #log.info("%s | % s", key,value)
            latency[key] = value
    ...
    if len(list(latency.values()))!=0:
        mean = sum(list(latency.values()))/len(list(latency.values()))
        variance = sum((i - mean) ** 2 for i in list(latency.values())) / len(list(latency.values()))
        std_dev = variance ** 0.5

    log.info("mean -%s, variance - %s, std-dev - %s",mean,variance,std_dev)
    ...
    if std_dev != 0:
        ...
        for key,value in latency.items():
            ...
            zscore_dict[key]=zscore
            log.info("%s | %s | %s",key,value,zscore)
    # CODE FOR DBSCAN
    values = list(latency.values())
    X = [[val] for val in values]
    clf = DBSCAN(eps=0.03, min_samples=5)

    # Fit and predict clusters
    clusters = clf.fit_predict(X)

    print("ANOMALIES DETECTED USING DBSCAN")
    for i, (key, val) in enumerate(zscore_dict.items()):
        if clusters[i] == -1 and zscore_dict[key] > 0:
            print(f"Key: {key}, Value: {val}")            
    """
    log.info("Zscore calculated")
    for key,value in link_latency.items():
        if len(key)==4 and type(value) != float:
            log.info("%s | % s", key,value)
    """
    if link not in self.adjacency:
        ...
        log.info('Links discovered until now:')
        for key,value in self.adjacency.items():
            log.info("%s | % s", key,value)
    else:
