...
class Discovery (EventMixin):
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if len(list(latency.values()))!=0:
        median= np.median(list(latency.values()))
        mad=np.median(np.abs(list(latency.values())-median))
    ...
    log.info("median -%s, mad - %s",median,mad)

    if mad != 0: 
        log.info("modified zscore: ")
        for key,value in latency.items():
            modified_z_scores = [(0.6745 * (value - median)) / mad]
            #zscore = (value-mean)/std_dev
            log.info("%s | %s | %s",key,value,modified_z_scores)
